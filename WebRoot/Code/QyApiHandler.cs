...
using System.Linq;
using System.Web;
using WeChatPlatform.WeChatAPI;
using WeChatPlatform.WeChatAPI.Entity;
...
namespace WebRoot.Code
{
    ...
    {
        private static QyApiHandler instance = null;
        public static QyApiHandler Instance { get { return instance; } }
        public static void Init(string CorpID, string Secret, string agentid)
        {
            instance = new QyApiHandler(CorpID, Secret, agentid);
        }
        ...
        public void RefreshToken()
        {
            ...
            {
                //QyAppDao qyAppDao = new QyAppDao();
                //qyAppDao.UpdateAccessToken(wechat.ID, rel.access_token);
            }
            else
            {
                throw new Exception("RefreshToken 系统异常");
            }
        ...
        {
            var rel = QyAPI.User_GetUserInfo(access_token, code);
            if (IsAccessTokenError(rel))
            {
                rel = QyAPI.User_GetUserInfo(access_token, code);
                IsAccessTokenError(rel, false);
            }
            return rel;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Get_jsapi_ticketRes Get_jsapi_ticket()
        {
            var rel = QyAPI.Get_jsapi_ticket(access_token);
            if (IsAccessTokenError(rel))
            {
                rel = QyAPI.Get_jsapi_ticket(access_token);
                IsAccessTokenError(rel, false);
            }
            return rel;
        }
        ...
        public Get_jsapi_ticketRes jsap_ticket = null;

        #region 获取Jsapi调用凭据签名
        /// <summary>
        /// 获取Jsapi调用凭据签名
        /// </summary>
